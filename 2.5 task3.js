...
let partitionOn = (pred, items) => {
  let partitionTrue = [];
  let partitionFalse = [];
  for (let i = 0; i < items.length; i++) {
    if (!pred(items[i])) {
      partitionFalse.push(items[i]);
    } else {
      partitionTrue.push(items[i]);
    }
  }
  items.splice(0);
  for (let x in partitionFalse) {
    items.push(partitionFalse[x]);
  }
  for (let y in partitionTrue) {
    items.push(partitionTrue[y]);
  }
  return partitionFalse.length;
};
